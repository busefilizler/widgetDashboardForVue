...
<template>
  <div :class="gridTemp" class="h-[300px] bg-white flex items-center flex-row w-full p-4">
    <div class="w-1/2 mb-4 h-full">
      <input type="text" v-model="display" readonly class="w-full h-full mb-2 text-center text-4xl text-violet-900 font-bold focus:ring-0 focus:outline-none" />
    </div>
<div class="flex flex-col items-center w-full">
  <div class="grid grid-cols-4 gap-2 w-full">
      ...
      <button @click="append('/')" class="p-2 bg-violet-300 rounded">/</button>
    ...
    <button @click="clearDisplay" class="mt-4 p-2 bg-violet-900 text-white rounded w-full">Clear</button>
</div>
  </div>
</template>
